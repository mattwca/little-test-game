...
public class AutoTileRenderer(EntityManager entityManager, GraphicsDevice graphicsDevice, SpriteBatch spriteBatch, ContentManager contentManager, Helper helper)
{
        ...
        foreach (var entity in mapEntities)
        {
            ...
            for (var i = 0; i < mapData.Length; i++)
            {
                for (var j = 0; j < mapData[i].Length; j++)
                {
                    ...
                    var texture = _mapTextures.TryGetOrAdd(mapDefinition.TileType, () => _contentManager.Load<Texture2D>(mapDefinition.TileTexturePath));

                    var sourceRectangle = new Rectangle(tileDefinition.X, tileDefinition.Y, mapDefinition.TileSize, mapDefinition.TileSize);
